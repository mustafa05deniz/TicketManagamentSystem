...
module.exports = {
	...
	list: async function () {
		...
		return new Promise(async (resolve, reject) => {
			...
			// query start
			await ticketModel.find({},{messages:0}).then(result => {
				console.log(result);
	...
	filter: async function (query) {
		...
		return new Promise(async (resolve, reject) => {
			var queryList = [];
			query.filter._id ? queryList.push({"_id":mongoose.Types.ObjectId(query.filter._id)}):null;
			query.filter.status ? queryList.push({"status":parseInt(query.filter.status)}):null;
			query.filter.email ? queryList.push({"userEmail":query.filter.email}):null;
			query.filter.title ? queryList.push({"title":query.filter.title}):null;
	...
	ticketCount:async function(){
		return new Promise(async (resolve, reject) => {
			// return async promises 
			// query start
			await ticketModel.countDocuments().then(result => {
				resolve(result); // resolve try block
			}).catch(err => { // resolve catch block 
				reject(err);
			});
